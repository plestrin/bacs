...
int main(void){
	char 			plaintext[] = "Hi I am an AES OFB test vector distributed on 4 128-bit blocks!";
	...
	mode_enc_ofb((blockCipher)aes128_encrypt, AES_BLOCK_NB_BYTE, (uint8_t*)plaintext, (uint8_t*)ciphertext, sizeof(plaintext), (void*)round_key_128, (uint8_t*)iv);

	printf("\nCiphertext OFB: ");
	fprintBuffer_raw(stdout, (char*)ciphertext, sizeof(plaintext));

 	mode_dec_ofb((blockCipher)aes128_encrypt, AES_BLOCK_NB_BYTE, (uint8_t*)ciphertext, (uint8_t*)deciphertext, sizeof(plaintext), (void*)round_key_128, (uint8_t*)iv);
