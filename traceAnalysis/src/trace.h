 ...
 * - utiliser un importeur pour créer cet object à partir du truc JSON - ne pas passer trop de temps
 * - utiliser comme source pour les loop - mesurer le gain sur le temps d'execution
 * - implémenter la nouvelle idée pour les boucles 
 * - utiliser comme source pour les traceFragments
 * - utiliser des tailles variables pour les accès memoire / registre dans l'extraction dans arguments - ou du moins identifier clairement là ou il faudrait interfacer un nouveau truc
 * - faire une stratégie d'inport dynamique pour la trace (ccol mais pas indispensable)
 * - écrire directement la trace dans le bon format (mesurer le gain en performance)
 * - enregistrer directement la trace dans le bon format (mesurer le gain ou pas en perfomance)
 * - prendre en charge toute les instructions rencontrées et les registres XMM MMX
