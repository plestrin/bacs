...
int32_t graph_copy(struct graph* graph_dst, struct graph* graph_src, int32_t(*node_clone)(void*,const void*), int32_t(*edge_clone)(void*,const void*)){
	...
	for (node_cursor = graph_get_head_node(graph_src); node_cursor != NULL; node_cursor = node_get_next(node_cursor)){
		...
		if (node_clone(node_get_data(new_node), node_get_data(node_cursor))){
			log_err("node_clone returned an error code");
			free(new_node);
			return -1;
		}
		...
		for (edge_cursor = node_get_head_edge_src(node_cursor); edge_cursor != NULL; edge_cursor = edge_get_next_src(edge_cursor)){
			if (edge_get_dst(edge_cursor)->ptr != NULL){
				...
				if (edge_clone(edge_get_data(new_edge), edge_get_data(edge_cursor))){
					log_err("edge_clone returned an error code");
					free(new_edge);
					return -1;
				}
...
struct graph* graph_clone(struct graph* graph_src, int32_t(*node_clone)(void*,const void*), int32_t(*edge_clone)(void*,const void*)){
	...
	if (graph != NULL){
		...
		if (graph_copy(graph, graph_src, node_clone, edge_clone)){
...
int32_t graph_concat(struct graph* graph_dst, struct graph* graph_src, int32_t(*node_clone)(void*,const void*), int32_t(*edge_clone)(void*,const void*)){
	...
	if (graph_copy(graph_dst, graph_src, node_clone, edge_clone)){
		log_err("unable to copy graph");
		return -1;
	}
