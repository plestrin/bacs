...
struct minPathInternal{
	struct minPathMultiStep multi_step;
	uint32_t 				dst;
	uint64_t 				mask;
	uint32_t 				sink;
	struct node* 			node;
	struct minPathInternal* next;
};

static inline void minPathInternal_init(struct minPathInternal* internal, struct node* node){
	internal->multi_step.step.edge 	= NULL;
	internal->multi_step.step.dir 	= PATH_INVALID;
	internal->multi_step.next 		= NULL;
	internal->dst 					= GRAPHMINPATH_INVALID_DST;
	internal->mask 					= 0;
	internal->sink 					= 0;
	internal->node 					= node;
	internal->next 					= NULL;
...
static inline void minPathInternal_add_step(struct minPathInternal* internal, struct edge* edge, enum minPathDirection dir){
	struct minPathMultiStep* step;

	if ((step = malloc(sizeof(struct minPathMultiStep))) == NULL){
		log_err("unable to allocate memory");
		return;
	}

	memcpy(step, &(internal->multi_step), sizeof(struct minPathMultiStep));
...
static inline void minPathInternal_clean(struct minPathInternal* internal){
	struct minPathMultiStep* step;
...
static uint32_t minPath_add_recursive(struct array* path_array, struct minPath* min_path, struct minPathStep* step, uint32_t nb_path){
	struct minPath 				fork;
	struct minPathInternal* 	node_meta_data;
	struct minPathMultiStep* 	multi_step;

	for ( ; step->edge != NULL; step = &(node_meta_data->multi_step.step)){
		if (array_add(min_path->step_array, step) < 0){
			log_err("unable to add element to array");
		}

		if (step->dir == PATH_SRC_TO_DST){
			node_meta_data = (struct minPathInternal*)(edge_get_src(step->edge)->ptr);
		}
		else{
			node_meta_data = (struct minPathInternal*)(edge_get_dst(step->edge)->ptr);
		}
		...
		for (multi_step = node_meta_data->multi_step.next; multi_step != NULL; multi_step = multi_step->next){
			...
			nb_path = minPath_add_recursive(path_array, &fork, &(multi_step->step), nb_path);
...
static uint32_t minPath_add(struct array* path_array, struct minPathInternal* internal, struct node* reached_node, uint32_t nb_path){
	...
	if (nb_path < GRAPHMINPATH_MAX_MIN_PATH){
		...
		min_path.reached_node = reached_node;

		return minPath_add_recursive(path_array, &min_path, &(internal->multi_step.step), nb_path);
	}
	else{
		return GRAPHMINPATH_MAX_MIN_PATH;
	}
...
int32_t graphMinPath_bfs(struct graph* graph, struct node** buffer_src, uint32_t nb_src, struct node** buffer_dst, uint32_t nb_dst, struct array* path_array, uint64_t(*get_mask)(uint64_t,struct node*,struct edge*,enum minPathDirection)){
	struct minPathInternal* 	internals;
	struct minPathInternal* 	curr_orbital;
	struct minPathInternal* 	next_orbital;
	uint32_t 					i;
	struct node* 				node_cursor;
	struct edge*				edge_cursor;
	struct minPathInternal* 	internal_cursor;
	uint32_t 					nb_min_path 		= 0;
	uint32_t 					dst;
	uint64_t 					mask;

	if ((internals = malloc(sizeof(struct minPathInternal) * graph->nb_node)) == NULL){
		log_err("unable to allocate memory");
		return -1;
	}
	...
	for (node_cursor = graph_get_head_node(graph), i = 0; i < graph->nb_node && node_cursor != NULL; node_cursor = node_get_next(node_cursor), i++){
		...
		minPathInternal_init(internals + i, node_cursor);
	}

	for (i = 0; i < nb_dst; i++){
		((struct minPathInternal*)buffer_dst[i]->ptr)->sink = 1;
	}

	for (i = 0, curr_orbital = NULL; i < nb_src; i++){
		internal_cursor = (struct minPathInternal*)buffer_src[i]->ptr;
		...
		if (internal_cursor->sink){
			nb_min_path = minPath_add(path_array, curr_orbital, buffer_src[i], nb_min_path);
		}
	}
	...
	for(dst = 1; curr_orbital != NULL && nb_min_path == 0; curr_orbital = next_orbital, dst ++){
		for(next_orbital = NULL; curr_orbital != NULL; curr_orbital = curr_orbital->next){
			...
			for (edge_cursor = node_get_head_edge_src(curr_orbital->node); edge_cursor != NULL; edge_cursor = edge_get_next_src(edge_cursor)){
				...
				internal_cursor = (struct minPathInternal*)node_cursor->ptr;

				if (internal_cursor->dst == GRAPHMINPATH_INVALID_DST){
					internal_cursor->multi_step.step.edge = edge_cursor;
					internal_cursor->multi_step.step.dir = PATH_SRC_TO_DST;

					if (internal_cursor->sink){
						nb_min_path = minPath_add(path_array, internal_cursor, node_cursor, nb_min_path);
					}
					...
					internal_cursor->mask |= mask;
					minPathInternal_add_step(internal_cursor, edge_cursor, PATH_SRC_TO_DST);
			...
			for (edge_cursor = node_get_head_edge_dst(curr_orbital->node); edge_cursor != NULL; edge_cursor = edge_get_next_dst(edge_cursor)){
				...
				if (internal_cursor->dst == GRAPHMINPATH_INVALID_DST){
					...
					internal_cursor->mask |= mask;
					minPathInternal_add_step(internal_cursor, edge_cursor, PATH_DST_TO_SRC);
	...
	for (i = 0; i < graph->nb_node; i++){
		minPathInternal_clean(internals + i);
	}
