...
int main(){
	...
	unsigned char 	mysterious_buffer[AES_BLOCK_SIZE];
	...
	AES_ctr128_encrypt((unsigned char*)plaintext, ciphertext, sizeof(plaintext), &ekey, iv_tmp, mysterious_buffer, &num);

	memcpy(iv_tmp, iv, AES_BLOCK_SIZE);
	AES_ctr128_encrypt(ciphertext, deciphertext, sizeof(ciphertext), &ekey, iv_tmp, mysterious_buffer, &num);
