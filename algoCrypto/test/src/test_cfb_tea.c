...
int main(void){
	...
	mode_enc_cfb((blockCipher)xtea_encrypt, TEA_BLOCK_NB_BYTE, (uint8_t*)plaintext, (uint8_t*)ciphertext, sizeof(plaintext), (void*)key, iv);

	printf("\nCiphertext CFB: ");
	fprintBuffer_raw(stdout, (char*)ciphertext, sizeof(plaintext));

 	mode_dec_cfb((blockCipher)xtea_encrypt, TEA_BLOCK_NB_BYTE, (uint8_t*)ciphertext, (uint8_t*)deciphertext, sizeof(plaintext), (void*)key, iv);
