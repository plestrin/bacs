...
static void irImporter_part_reg_variable(struct ir* ir, struct alias* alias_src, struct alias* alias_dst, struct operand* operand, uint8_t alias_dst_size, enum irOpcode part_op);
static void irImporter_part_mem_variable(struct irRenameEngine* engine, struct alias* alias_src, struct node** ir_node, struct operand* operand, enum irOpcode part_op);
...
static void irImporter_merge_mem_variable(struct irRenameEngine* engine, struct alias* alias_dst, struct operand* operand, enum irOpcode part_op);
...
static void irImporter_part_reg_variable(struct ir* ir, struct alias* alias_src, struct alias* alias_dst, struct operand* operand, uint8_t alias_dst_size, enum irOpcode part_op){
	...
	alias_dst->alias_type.reg.ir_node 	= irImporterAsm_add_operation(ir, part_op, operand, alias_dst_size);
...
static void irImporter_part_mem_variable(struct irRenameEngine* engine, struct alias* alias_src, struct node** ir_node, struct operand* operand, enum irOpcode part_op){
	...
	*ir_node = irImporterAsm_add_operation(engine->ir, part_op, operand, operand->size * 8);
	if (*ir_node == NULL){
	...
	irImporterAsm_add_dependence(engine->ir, alias_src->alias_type.mem.ir_node, *ir_node, IR_DEPENDENCE_TYPE_DIRECT);
...
static void irImporter_merge_mem_variable(struct irRenameEngine* engine, struct alias* alias_dst, struct operand* operand, enum irOpcode part_op){
	...
	ir_node = irImporterAsm_add_input(engine->ir, operand, operand->size * 8);
	if (ir_node == NULL){
		printf("ERROR: in %s, unable to add input to IR\n", __func__);
		alias_free(engine, alias_new);
		return;
	...
	if (alias_dst->alias_type.mem.prev == NULL){
		...
		irImporterAsm_add_dependence(engine->ir, alias_dst->alias_type.mem.ir_node, alias_new->alias_type.mem.ir_node, IR_DEPENDENCE_TYPE_DIRECT);
	}
	else{
		...
		if (alias_result != NULL){
			...
			if (node == NULL){
				if ((*alias_result)->alias_type.mem.size > operand->size){
					...
					irImporter_part_mem_variable(engine, *alias_result, &node, operand, IR_PART1_8);
				}
				else{
					/* pour le debug */
					printf("WARNING: in %s, this is a border case (merge mem variable) be careful lad\n", __func__);
					irImporter_merge_mem_variable(engine, *alias_result, operand, IR_PART1_8);
					node = (*alias_result)->alias_type.mem.ir_node;
				}
			}
		}
			...
			switch(reg){
				case REGISTER_EAX 	: {
					if (engine->register_table[REGISTER_AH - 1].alias_type.reg.ir_node != NULL){
						if (ALIAS_IS_READ(engine->register_table[REGISTER_AH - 1].type)){
							irImporter_merge_reg_variable(engine->ir, engine->register_table + (REGISTER_EAX - 1), engine->register_table + (REGISTER_AH - 1), operand, 32, IR_PART2_8);
						}
						else{
					...
					if (engine->register_table[REGISTER_AL - 1].alias_type.reg.ir_node != NULL){
						if (ALIAS_IS_READ(engine->register_table[REGISTER_AL - 1].type)){
							irImporter_merge_reg_variable(engine->ir, engine->register_table + (REGISTER_EAX - 1), engine->register_table + (REGISTER_AL - 1), operand, 32, IR_PART1_8);
						}
						else{
				...
				case REGISTER_AH 	: {
					if (engine->register_table[REGISTER_EAX - 1].alias_type.reg.ir_node != NULL){
						irImporter_part_reg_variable(engine->ir, engine->register_table + (REGISTER_EAX - 1), engine->register_table + (REGISTER_AH - 1), operand, 8, IR_PART2_8);
					}
				...
				case REGISTER_AL 	: {
					if (engine->register_table[REGISTER_EAX - 1].alias_type.reg.ir_node != NULL){
						irImporter_part_reg_variable(engine->ir, engine->register_table + (REGISTER_EAX - 1), engine->register_table + (REGISTER_AL - 1), operand, 8, IR_PART1_8);
					}
				...
				case REGISTER_EBX 	: {
					if (engine->register_table[REGISTER_BH - 1].alias_type.reg.ir_node != NULL){
						if (ALIAS_IS_READ(engine->register_table[REGISTER_BH - 1].type)){
							irImporter_merge_reg_variable(engine->ir, engine->register_table + (REGISTER_EBX - 1), engine->register_table + (REGISTER_BH - 1), operand, 32, IR_PART2_8);
						}
						else{
					...
					if (engine->register_table[REGISTER_BL - 1].alias_type.reg.ir_node != NULL){
						if (ALIAS_IS_READ(engine->register_table[REGISTER_BL - 1].type)){
							irImporter_merge_reg_variable(engine->ir, engine->register_table + (REGISTER_EBX - 1), engine->register_table + (REGISTER_BL - 1), operand, 32, IR_PART1_8);
						}
						else{
				...
				case REGISTER_BH 	: {
					if (engine->register_table[REGISTER_EBX - 1].alias_type.reg.ir_node != NULL){
						irImporter_part_reg_variable(engine->ir, engine->register_table + (REGISTER_EBX - 1), engine->register_table + (REGISTER_BH - 1), operand, 8, IR_PART2_8);
					}
				...
				case REGISTER_BL 	: {
					if (engine->register_table[REGISTER_EBX - 1].alias_type.reg.ir_node != NULL){
						irImporter_part_reg_variable(engine->ir, engine->register_table + (REGISTER_EBX - 1), engine->register_table + (REGISTER_BL - 1), operand, 8, IR_PART1_8);
					}
				...
				case REGISTER_ECX 	: {
					if (engine->register_table[REGISTER_CH - 1].alias_type.reg.ir_node != NULL){
						if (ALIAS_IS_READ(engine->register_table[REGISTER_CH - 1].type)){
							irImporter_merge_reg_variable(engine->ir, engine->register_table + (REGISTER_ECX - 1), engine->register_table + (REGISTER_CH - 1), operand, 32, IR_PART2_8);
						}
						else{
					...
					if (engine->register_table[REGISTER_CL - 1].alias_type.reg.ir_node != NULL){
						if (ALIAS_IS_READ(engine->register_table[REGISTER_CL - 1].type)){
							irImporter_merge_reg_variable(engine->ir, engine->register_table + (REGISTER_ECX - 1), engine->register_table + (REGISTER_CL - 1), operand, 32, IR_PART1_8);
						}
						else{
				...
				case REGISTER_CH 	: {
					if (engine->register_table[REGISTER_ECX - 1].alias_type.reg.ir_node != NULL){
						irImporter_part_reg_variable(engine->ir, engine->register_table + (REGISTER_ECX - 1), engine->register_table + (REGISTER_CH - 1), operand, 8, IR_PART2_8);
					}
				...
				case REGISTER_CL 	: {
					if (engine->register_table[REGISTER_ECX - 1].alias_type.reg.ir_node != NULL){
						irImporter_part_reg_variable(engine->ir, engine->register_table + (REGISTER_ECX - 1), engine->register_table + (REGISTER_CL - 1), operand, 8, IR_PART1_8);
					}
				...
				case REGISTER_EDX 	: {
					if (engine->register_table[REGISTER_DH - 1].alias_type.reg.ir_node != NULL){
						if (ALIAS_IS_READ(engine->register_table[REGISTER_DH - 1].type)){
							irImporter_merge_reg_variable(engine->ir, engine->register_table + (REGISTER_EDX - 1), engine->register_table + (REGISTER_DH - 1), operand, 32, IR_PART2_8);
						}
						else{
					...
					if (engine->register_table[REGISTER_DL - 1].alias_type.reg.ir_node != NULL){
						if (ALIAS_IS_READ(engine->register_table[REGISTER_DL - 1].type)){
							irImporter_merge_reg_variable(engine->ir, engine->register_table + (REGISTER_EDX - 1), engine->register_table + (REGISTER_DL - 1), operand, 32, IR_PART1_8);
						}
						else{
				...
				case REGISTER_DH 	: {
					if (engine->register_table[REGISTER_EDX - 1].alias_type.reg.ir_node != NULL){
						irImporter_part_reg_variable(engine->ir, engine->register_table + (REGISTER_EDX - 1), engine->register_table + (REGISTER_DH - 1), operand, 8, IR_PART2_8);
					}
				...
				case REGISTER_DL 	: {
					if (engine->register_table[REGISTER_EDX - 1].alias_type.reg.ir_node != NULL){
						irImporter_part_reg_variable(engine->ir, engine->register_table + (REGISTER_EDX - 1), engine->register_table + (REGISTER_DL - 1), operand, 8, IR_PART1_8);
					}
