...
static int32_t codeSignatureBuilder_init(struct codeSignatureBuilder* builder, struct codeSignatureCollection* collection){
	graph_init(&(builder->code_signature.graph), sizeof(struct signatureNode), sizeof(struct signatureEdge));
	
	builder->is_name_set = 0;
	builder->collection = collection;

	if ((builder->symbol_set = set_create(CODESIGNATURE_NAME_MAX_SIZE, 4)) == NULL){
...
static void codeSignatureReader_handle_graph_name(const char* str, size_t str_len, void* arg){
	...
	memset(builder->code_signature.name, 0, CODESIGNATURE_NAME_MAX_SIZE);
	memcpy(builder->code_signature.name, str, min(CODESIGNATURE_NAME_MAX_SIZE - 1, str_len));

	memset(builder->code_signature.symbol, 0, CODESIGNATURE_NAME_MAX_SIZE);
	memcpy(builder->code_signature.symbol, str, min(CODESIGNATURE_NAME_MAX_SIZE - 1, str_len));
...
static void* codeSignatureReader_handle_new_node(void* arg){
	...
	if ((node = graph_add_node_(&(builder->code_signature.graph))) == NULL){
		log_err("unable to add node to graph");
	}
	else{
		signature_node = (struct signatureNode*)&(node->data);
		signature_node->type 				= SIGNATURE_NODE_TYPE_INVALID;
...
static void codeSignatureReader_handle_node_label(const char* str, size_t str_len, void* ptr, void* arg){
	...
	if (signature_node->type != SIGNATURE_NODE_TYPE_INVALID){
		log_warn_m("the label has already been set for this node %s (signature: %s)", (signature_node->type == SIGNATURE_NODE_TYPE_OPCODE) ? irOpcode_2_string(signature_node->node_type.opcode): "", builder->is_name_set ? builder->code_signature.name : "?");
	}

	if (!strncmp(str, "ADD", str_len)){
		signature_node->type = SIGNATURE_NODE_TYPE_OPCODE;
		signature_node->node_type.opcode = IR_ADD;
		return;
	}
	else if (!strncmp(str, "AND", str_len)){
		signature_node->type = SIGNATURE_NODE_TYPE_OPCODE;
		signature_node->node_type.opcode = IR_AND;
		return;
	}
	else if (!strncmp(str, "MUL", str_len)){
		signature_node->type = SIGNATURE_NODE_TYPE_OPCODE;
		signature_node->node_type.opcode = IR_MUL;
		return;
	}
	else if (!strncmp(str, "MOVZX", str_len)){
		signature_node->type = SIGNATURE_NODE_TYPE_OPCODE;
		signature_node->node_type.opcode = IR_MOVZX;
		return;
	}
	else if (!strncmp(str, "NOT", str_len)){
		signature_node->type = SIGNATURE_NODE_TYPE_OPCODE;
		signature_node->node_type.opcode = IR_NOT;
		return;
	}
	else if (!strncmp(str, "OR", str_len)){
		signature_node->type = SIGNATURE_NODE_TYPE_OPCODE;
		signature_node->node_type.opcode = IR_OR;
		return;
	}
	else if (!strncmp(str, "ROR", str_len)){
		signature_node->type = SIGNATURE_NODE_TYPE_OPCODE;
		signature_node->node_type.opcode = IR_ROR;
		return;
	}
	else if (!strncmp(str, "SHL", str_len)){
		signature_node->type = SIGNATURE_NODE_TYPE_OPCODE;
		signature_node->node_type.opcode = IR_SHL;
		return;
	}
	else if (!strncmp(str, "SHR", str_len)){
		signature_node->type = SIGNATURE_NODE_TYPE_OPCODE;
		signature_node->node_type.opcode = IR_SHR;
		return;
	}
	else if (!strncmp(str, "SUB", str_len)){
		signature_node->type = SIGNATURE_NODE_TYPE_OPCODE;
		signature_node->node_type.opcode = IR_SUB;
		return;
	}
	else if (!strncmp(str, "XOR", str_len)){
		signature_node->type = SIGNATURE_NODE_TYPE_OPCODE;
		signature_node->node_type.opcode = IR_XOR;
		return;
	}
	else if (!strncmp(str, "LOAD", str_len)){
		signature_node->type = SIGNATURE_NODE_TYPE_OPCODE;
		signature_node->node_type.opcode = IR_LOAD;
		return;
	}
	else if (!strncmp(str, "STORE", str_len)){
		signature_node->type = SIGNATURE_NODE_TYPE_OPCODE;
		signature_node->node_type.opcode = IR_STORE;
		return;
	}
	else if (!strncmp(str, "*", str_len)){
		signature_node->type = SIGNATURE_NODE_TYPE_OPCODE;
		signature_node->node_type.opcode = IR_JOKER;
		return;
	}

	memset(symbol, 0, CODESIGNATURE_NAME_MAX_SIZE);
	memcpy(symbol, str, min(CODESIGNATURE_NAME_MAX_SIZE - 1, str_len));
	
	if ((uint32_t)(signature_node->node_type.symbol = (void*)set_add_unique(builder->symbol_set, symbol)) & 0x80000000){
		log_err("unable to add element to array");
	}
	else{
		signature_node->type = SIGNATURE_NODE_TYPE_SYMBOL;	
	}
}
...
static void codeSignatureReader_handle_node_io(const char* str, size_t str_len, void* ptr, void* arg){
	...
	if (!memcmp(str, "I:", 2)){
		if (signature_node->input_number | signature_node->input_frag_order){
			log_warn("the IO tag has already been set for this node");
		}

		for (i = 2; i < str_len - 1; i++){
			if (str[i] == ':'){
				signature_node->input_number = atoi(str + 2);
				signature_node->input_frag_order = atoi(str + i + 1);
				return;
			}
		}
		log_err("incorrect IO tag");
		...
		if (signature_node->output_number | signature_node->output_frag_order){
			log_warn("the IO tag has already been set for this node");
		}

		for (i = 2; i < str_len - 1; i++){
			if (str[i] == ':'){
				signature_node->output_number = atoi(str + 2);
				signature_node->output_frag_order = atoi(str + i + 1);
				return;
			}
		}
		log_err("incorrect IO tag");
...
static void* codeSignatureReader_handle_new_edge(void* src, void* dst, void* arg){
	...
	if ((edge = graph_add_edge_(&(builder->code_signature.graph), (struct node*)src, (struct node*)dst)) == NULL){
		log_err("unable to add edge to graph");
	}
	else{
		signature_edge = (struct signatureEdge*)&(edge->data);
...
static void codeSignatureReader_handle_edge_label(const char* str, size_t str_len, void* ptr, void* arg){
	...
	if (str_len == 1 && str[0] == '@'){
		signature_edge->type = IR_DEPENDENCE_TYPE_ADDRESS;
		return;
	}
	...
	if (str_len >= 4 && (str[0] == 'I' || str[0] == 'O')){
		...
		if (nb_digit1 > 0 && str_len > 2 + nb_digit1 && nb_digit1 > 0 && str[1 + nb_digit1] == 'F'){
			...
			if (nb_digit2 > 0){
				if (str[0] == 'I'){
					signature_edge->type = IR_DEPENDENCE_TYPE_MACRO;
					signature_edge->macro_desc = IR_DEPENDENCE_MACRO_DESC_SET_INPUT(atoi(str + 2 + nb_digit1), atoi(str + 1));
				}
				else{
					signature_edge->type = IR_DEPENDENCE_TYPE_MACRO;
					signature_edge->macro_desc = IR_DEPENDENCE_MACRO_DESC_SET_OUTPUT(atoi(str + 2 + nb_digit1), atoi(str + 1));
				}
				return;
			}
		}
...
static void codeSignatureReader_handle_flush_graph(void* arg){
	...
	if ((symbol_table = (struct signatureSymbolTable*)malloc(signatureSymbolTable_get_size(set_get_length(builder->symbol_set)))) == NULL){
		...
		graph_clean(&(builder->code_signature.graph));
		goto exit;
	}
	...
	for (symbol_ptr = (char*)setIterator_get_first(builder->symbol_set, &iterator), i = 0; symbol_ptr != NULL; symbol_ptr = setIterator_get_next(&iterator), i++){
		...
		memcpy(symbol_table->symbols[i].name, symbol_ptr, CODESIGNATURE_NAME_MAX_SIZE);	
	}

	for (node_cursor = graph_get_head_node(&(builder->code_signature.graph)); node_cursor != NULL; node_cursor = node_get_next(node_cursor)){
		signature_node = (struct signatureNode*)&(node_cursor->data);

		if (signature_node->type == SIGNATURE_NODE_TYPE_INVALID){
			log_warn("a node label has not been set");
		}
		else if (signature_node->type == SIGNATURE_NODE_TYPE_SYMBOL){
			signature_node->node_type.symbol = symbol_table->symbols + (uint32_t)signature_node->node_type.symbol;
		}
	}
	...
	builder->code_signature.sub_graph_handle 	= NULL;
	builder->code_signature.symbol_table 		= symbol_table;

	if (codeSignatureCollection_add_codeSignature(builder->collection, &(builder->code_signature))){
